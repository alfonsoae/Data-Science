...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        thePatient = Patient(viruses, maxPop)

        for j in range(tSteps):
            thePatient.update()
            avgSize[j] += thePatient.getTotalPop()

    avgSize = [avgSize[i]/numTrials for i in range(tSteps)]

    pylab.plot(avgSize, label="SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        """
        resistances = {}
        willReproduce = [self.isResistantTo(drug) for drug in activeDrugs]

        if False not in willReproduce and random.random() <= self.maxBirthProb * (1 - popDensity):
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        post-condition: The list of drugs being administered to a patient is updated
        """
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.getViruses():
            try:
                if False not in [virus.getResistances()[drug] for drug in drugResist]:
                    resistant += 1
            except KeyError:
                continue
    ...
    def update(self):
        ...
        for virus in self.getViruses():
            try:
                temp.append(virus.reproduce(popDensity, self.getPrescriptions()))
            except NoChildException:
                continue

        self.viruses = self.viruses + temp
        return self.getTotalPop()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        thePatient = TreatedPatient(viruses, maxPop)

        for j in range(tSteps):
            if j == 150:
                thePatient.addPrescription("guttagonol")
            thePatient.update()
            avgTotal[j] += thePatient.getTotalPop()
            avgResist[j] += thePatient.getResistPop(["guttagonol"])

    avgTotal = [avgTotal[i]/numTrials for i in range(tSteps)]
    avgResist = [avgResist[i]/numTrials for i in range(tSteps)]

    pylab.plot(avgTotal, label="Total")
    pylab.plot(avgResist, label="guttagonol")
